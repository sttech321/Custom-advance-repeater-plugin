...
class Ultimate_Repeater_Field {
            ...
            function createNestedSubfieldRow(parentIndex, subIndex, nestedIndex) {
                ...
                div.innerHTML = `
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-bottom: 10px;">
                        ...
                        <div>
                            <label><?php _e('Field Label', 'ultimate-repeater-field'); ?> *</label>
                            <input type="text" name="fields[${parentIndex}][subfields][${subIndex}][subfields][${nestedIndex}][label]" class="widefat" required>
                        </div>
                        
                        <div>
                            <label><?php _e('Field Name', 'ultimate-repeater-field'); ?> *</label>
                            <input type="text" name="fields[${parentIndex}][subfields][${subIndex}][subfields][${nestedIndex}][name]" class="widefat" required>
                            <p class="description"><?php _e('Lowercase, underscores, no spaces', 'ultimate-repeater-field'); ?></p>
                        </div>
                    </div>
    ...
    /* FIXED: Ensure table cells display content properly */
    .urf-nested-table td > * {
        width: 100%;
        max-width: 100%;
    }

    .urf-nested-table td input[type="text"],
    .urf-nested-table td textarea,
    .urf-nested-table td select {
        width: 100%;
        box-sizing: border-box;
        padding: 10px 12px;
        border: 2px solid #e2e8f0;
    ...
    .urf-nested-table td .urf-file-upload-container {
        margin-top: 0;
    }

    .urf-nested-table td .urf-upload-button {
    ...
    /* Better spacing for nested repeaters inside table cells */
    .urf-nested-table td .urf-nested-repeater {
        margin-top: 10px;
        padding: 15px;
    }
    </style>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    (function($) {
        ...
        function initNestedRepeaters() {
            // Add level 1 nested row
            $(document).on('click', '.urf-add-nested-row', function(e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                // Reinitialize datepickers and colorpickers
                setTimeout(function() {
                    ...
                    // Reinitialize image upload for new row
                    $newRow.find('.urf-upload-button').each(function() {
                        $(this).off('click').on('click', function(e) {
                            e.preventDefault();
                            e.stopPropagation();
                            
                            initImageUploadForButton($(this));
                        });
                    });
                }, 100);
            });
            
            // Remove level 1 nested row
            $(document).on('click', '.urf-remove-nested-row', function(e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                if (confirm('<?php _e('Are you sure you want to remove this nested row?', 'ultimate-repeater-field'); ?>')) {
                    ...
                    $tbody.find('tr:not(.urf-clone-nested-row)').each(function(newIndex) {
                        var $currentRow = $(this);
                        var oldIndex = parseInt($currentRow.data('nested-index'));
                        ...
                        $currentRow.find('[name]').each(function() {
                            var name = $(this).attr('name');
                            if (name) {
                                // Update the nested index in the name
                                var newName = name.replace(/\[(\d+)\]/g, function(match, p1, offset, fullString) {
                                    // Get the context of this replacement
                                    var parts = fullString.split('[');
                                    var currentPartIndex = 0;
                                    
                                    for (var i = 0; i < parts.length; i++) {
                                        if (fullString.indexOf('[' + p1 + ']') < fullString.indexOf(parts[i] + '[')) {
                                            currentPartIndex++;
                                        }
                                    }
                                    
                                    // Usually the second index is the row index
            ...
            urfLog('Level 1 nested repeaters initialized');
        }
        ...
        function initNested2Repeaters() {
            ...
            $(document).on('click', '.urf-add-nested2-row', function(e) {
                ...
                var nested2RowCount = $tbody.find('tr:not(.urf-clone-nested2-row)').length;
                var $newRow = $cloneRow.clone();
                ...
                $newRow.find('[name]').each(function() {
                    var name = $(this).attr('name');
                    if (name) {
                        name = name.replace(/\[__NESTED2_INDEX__\]/g, '[' + nested2RowCount + ']');
                        $(this).attr('name', name);
                    }
                });
            ...
            $(document).on('click', '.urf-remove-nested2-row', function(e) {
                ...
                if (confirm('<?php _e('Are you sure you want to remove this nested row?', 'ultimate-repeater-field'); ?>')) {
                    ...
                    $tbody.find('tr:not(.urf-clone-nested2-row)').each(function(newIndex) {
                        var $currentRow = $(this);
                        var oldIndex = parseInt($currentRow.data('nested2-index'));
                        ...
                        $currentRow.find('[name]').each(function() {
                            var name = $(this).attr('name');
                            if (name) {
                                name = name.replace(/\[(\d+)\]/g, function(match, p1, offset, fullString) {
                                    // Get the context of this replacement
                                    var parts = fullString.split('[');
                                    var currentPartIndex = 0;
                                    ...
                                    // For nested2, it's usually the third index
        ...
        // Helper function for image upload initialization
        function initImageUploadForButton($button) {
...
public function render_nested2_repeater_row($subfields, $field_name, $group_slug, $parent_field_name, $parent_row_index, $nested2_index, $nested2_row, $is_clone = false) {
    ...
    foreach ($_POST['urf_data'] as $group_slug => $data) {
        ...
        foreach ($repeater_fields as $repeater_name => $_) {
            ...
            // IMPROVED: Better filtering of empty rows
            $filtered_data = array();
            foreach ($data[$repeater_name] as $row_index => $row) {
                // Skip placeholder keys
                if ($row_index === '__INDEX__' || $row_index === '__NESTED_INDEX__') {
                    continue;
                }
                ...
                foreach ($row as $field_name => $field_value) {
                    // For nested repeaters (level 2)
                    if (is_array($field_value)) {
                        $has_nested_content = false;
                        $clean_nested_data = array();
                        
                        foreach ($field_value as $nested_index => $nested_row) {
                            // Skip nested placeholder keys
                            if ($nested_index === '__NESTED2_INDEX__') {
                                continue;
                            }
                            ...
                            // Check if nested row has content
                            $nested_has_content = false;
                            $clean_nested_row = array();
                            
                            foreach ($nested_row as $nested_field_name => $nested_field_value) {
                                if (!empty($nested_field_value) && $nested_field_value !== '' && $nested_field_value !== null) {
                                    $nested_has_content = true;
                                    $clean_nested_row[$nested_field_name] = $nested_field_value;
                                }
                            }
                            
                            if ($nested_has_content) {
                                $has_nested_content = true;
                                $clean_nested_data[$nested_index] = $clean_nested_row;
                            }
                        }
                        
                        if ($has_nested_content) {
                            $has_content = true;
                            $clean_row[$field_name] = $clean_nested_data;
                        }
                    } 
                    // For regular fields
                ...
                if ($has_content) {
                    $filtered_data[$row_index] = $clean_row;
                }
            }

            // If no valid rows, skip saving
            if (empty($filtered_data)) {
                continue;
            }

            // Sort by numeric keys
            ksort($filtered_data, SORT_NUMERIC);
            $nested_order = 0;

            foreach ($filtered_data as $nested_index => $nested_row) {
                $row_has_content = false;
                
                foreach ($nested_row as $sub_name => $sub_value) {
                    // Handle nested repeaters (level 2)
                    if (is_array($sub_value) && !empty($sub_value)) {
                        foreach ($sub_value as $nested2_index => $nested2_row) {
                            foreach ($nested2_row as $nested2_sub_name => $nested2_sub_value) {
                                if (empty($nested2_sub_value) && $nested2_sub_value !== '0' && $nested2_sub_value !== 0) {
                                    continue;
                                }
                                ...
                                $wpdb->insert($table_name, [
                                    ...
                                    'row_index'        => $nested2_index,
                                    'field_name'       => $nested2_sub_name,
                                    'field_type'       => 'nested2',
                                    'field_value'      => is_array($nested2_sub_value)
                                        ? maybe_serialize($nested2_sub_value)
                                        : (string) $nested2_sub_value,
                                    'parent_field'     => $sub_name,
                    ...
                    // Handle regular nested fields
                        ...
                        if (empty($sub_value) && $sub_value !== '0' && $sub_value !== 0) {
                            continue;
                        }
                ...
                // Only increment if row had content
                if ($row_has_content) {
                    $nested_order++;
                }
            }
