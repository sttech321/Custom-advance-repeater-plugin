document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof carf_field_group_config !== 'undefined') {
        (function($) {
            let fieldIndex = carf_field_group_config.fields_count;
            ...
            function toggleDisplayOptions() {
                const selected = document.querySelector('input[name="display_logic"]:checked');
                if (!selected) return;
                ...
                if (selectedValue === 'post_types') {
                    ...
                    const allSelected = carf_field_group_config.is_all_types;
            ...
            if (allPostTypesCheckbox) {
                ...
                if (carf_field_group_config.is_all_types) {
                    allPostTypesCheckbox.checked = true;
                    postTypesContainer.style.display = "none";
                    pagesSection.style.display = "none";
                }
            ...
            function loadPages(search = '') {
                pagesList.innerHTML = '<p>' + carf_field_group_config.i18n.loading + '</p>';

                const data = new FormData();
                data.append('action', 'carf_get_pages');
                data.append('search', search);
                data.append('nonce', carf_admin_vars.ajax_nonce);

                fetch(carf_admin_vars.ajax_url, {
                        method: 'POST',
                        body: data
                    })
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            if (data.data.length > 0) {
                                ...
                                pagesList.innerHTML = '<p>' + carf_field_group_config.i18n.no_pages + '</p>';
                            }
                        } else {
                            pagesList.innerHTML = '<p>' + carf_field_group_config.i18n.error + '</p>';
                        }
                    })
                    .catch(error => {
                        pagesList.innerHTML = '<p>' + carf_field_group_config.i18n.error + '</p>';
            ...
            if (clearPagesBtn) {
                clearPagesBtn.addEventListener('click', function() {
                    if (confirm(carf_field_group_config.i18n.confirm_clear)) {
            ...
            // Add field
            const addFieldBtn = document.getElementById('carf-add-field');
            if (addFieldBtn) {
                addFieldBtn.addEventListener('click', function() {
                    const container = document.getElementById('carf-fields-container');
            ...
            document.addEventListener('click', function(e) {
                if (e.target.closest('.carf-remove-field')) {
                    e.preventDefault();
                    if (confirm(carf_field_group_config.i18n.confirm_remove_field)) {
                        const row = e.target.closest('.carf-field-row');
                        row.remove();
                        updateFieldIndices();
                    }
                }
            ...
            // Remove subfield
            document.addEventListener('click', function(e) {
                if (e.target.closest('.carf-remove-subfield')) {
                    e.preventDefault();
                    if (confirm(carf_field_group_config.i18n.confirm_remove_subfield)) {
                        const row = e.target.closest('.carf-subfield-row');
                        row.remove();
                    }
                }
            });
            ...
            document.addEventListener('click', function(e) {
                if (e.target.closest('.carf-remove-nested-subfield')) {
                    e.preventDefault();
                    if (confirm(carf_field_group_config.i18n.confirm_remove_nested)) {
                        const row = e.target.closest('.carf-nested-subfield-row');
                        row.remove();
                    }
                }
            });
            ...
            function togglePagesContainer() {
                if (selectedPagesContainer) {
                    if (selectedPagesContainer.querySelectorAll('.selected-page').length > 0) {
            ...
            function handleLabelToNameConversion(target) {
                ...
                if (target.classList.contains('carf-field-label')) {
                    isLabelField = true;
                    fieldLevel = 0;
                ...
                switch (fieldLevel) {
                    case 0:
                        const row = target.closest('.carf-field-row');
                        nameField = row?.querySelector('.carf-field-name');
                        break;
                    case 1:
                        const subRow = target.closest('.carf-subfield-row');
                        nameField = subRow?.querySelector('input[name*="[name]"]');
                        break;
                    case 2:
                        const nestedRow = target.closest('.carf-nested-subfield-row');
            ...
            // Show/hide options based on field type (LEVEL 0)
            document.addEventListener('change', function(e) {
                if (e.target.classList.contains('carf-field-type')) {
                    const row = e.target.closest('.carf-field-row');
                    const type = e.target.value;
                    const optionsDiv = row.querySelector('.carf-field-options');

                    if (['select', 'checkbox', 'radio', 'repeater'].includes(type)) {
                        optionsDiv.style.display = 'block';

                        // Only inject HTML if it's missing or if we are switching types (optional optimization)
                        // For simplicity and to fix the bug, we ensure specific HTML is there.
                        if (['select', 'checkbox', 'radio'].includes(type)) {
                            // Check if textarea exists to avoid overwriting saved values on init
                            if (!optionsDiv.querySelector('textarea')) {
                                const currentIndex = Array.from(document.querySelectorAll('.carf-field-row')).indexOf(row);
                                optionsDiv.innerHTML = `
                                    <label>${carf_field_group_config.i18n.options_label}</label>
                                    <textarea name="fields[${currentIndex}][options]" class="widefat" rows="5" placeholder="My Option 1"></textarea>
                                    <p class="description">${carf_field_group_config.i18n.options_desc}</p>
                                `;
                            }
                        } else if (type === 'repeater') {
                            if (!optionsDiv.querySelector('.carf-subfields-container')) {
                                const currentIndex = Array.from(document.querySelectorAll('.carf-field-row')).indexOf(row);
                                optionsDiv.innerHTML = `
                                    <label style=" font-size: 16px;font-weight: 600;">${carf_field_group_config.i18n.sub_fields_label}</label>
                                    <div class="carf-subfields-container" style="margin-top: 6px" data-parent-index="${currentIndex}">
                                    </div>
                                    <button type="button" class="button button-small carf-add-subfield add_field_btn" data-parent-index="${currentIndex}">
                                        <span class="dashicons dashicons-plus"></span> ${carf_field_group_config.i18n.add_sub_field}
                                    </button>
                                    <p class="description">${carf_field_group_config.i18n.add_fields_desc}</p>
                                `;
                            }
                        }
                    } else {
            ...
            // Handle subfield type change (LEVEL 1)
            document.addEventListener('change', function(e) {
                if (e.target.classList.contains('carf-subfield-type')) {
                    const row = e.target.closest('.carf-subfield-row');
                    const type = e.target.value;
                    const optionsDiv = row.querySelector('.carf-subfield-options');
                    ...
                    if (['select', 'checkbox', 'radio', 'repeater'].includes(type)) {
                        ...
                        if (type === 'repeater') {
                            if (!optionsDiv.querySelector('.carf-subfields-container')) {
                                const fieldRow = row.closest('.carf-field-row');
                                const parentIndex = Array.from(document.querySelectorAll('.carf-field-row')).indexOf(fieldRow);
                                const subIndex = Array.from(fieldRow.querySelectorAll('.carf-subfield-row')).indexOf(row);

                                optionsDiv.innerHTML = `
                                    <label style=" font-size: 16px;font-weight: 600;">${carf_field_group_config.i18n.sub_fields_label}</label>
                                    <div class="carf-subfields-container" data-parent-index="${parentIndex}" data-sub-index="${subIndex}">
                                    </div>
                                    <button type="button" class="button button-small carf-add-nested-subfield add_field_btn" data-parent-index="${parentIndex}" data-sub-index="${subIndex}">
                                        <span class="dashicons dashicons-plus"></span> ${carf_field_group_config.i18n.add_sub_field}
                                    </button>
                                    <p class="description">${carf_field_group_config.i18n.add_fields_desc}</p>
                                `;
                            ...
                            // FIX: Logic for select/checkbox/radio in Level 1
                            if (!optionsDiv.querySelector('textarea')) {
                                const nameAttr = e.target.name.replace('[type]', '[options]');
                                optionsDiv.innerHTML = `
                                    <label>${carf_field_group_config.i18n.options_label}</label>
                                    <textarea name="${nameAttr}" class="widefat" rows="5" placeholder="Option 1\nOption 2"></textarea>
                                    <p class="description">${carf_field_group_config.i18n.options_desc}</p>
                                `;
                            }
                        }
            ...
            // Handle Nested subfield type change (LEVEL 2)
            document.addEventListener('change', function(e) {
                if (e.target.classList.contains('carf-nested-subfield-type')) {
                    const row = e.target.closest('.carf-nested-subfield-row');
                    const type = e.target.value;
                    const optionsDiv = row.querySelector('.carf-nested-subfield-options');

                    if (['select', 'checkbox', 'radio'].includes(type)) {
                        optionsDiv.style.display = 'block';
                        // FIX: Logic for select/checkbox/radio in Level 2
                        if (!optionsDiv.querySelector('textarea')) {
                            const nameAttr = e.target.name.replace('[type]', '[options]');
                            optionsDiv.innerHTML = `
                                <label>${carf_field_group_config.i18n.options_label}</label>
            ...
            // Add subfield (FIXED for undefined error)
            document.addEventListener('click', function(e) {
                if (e.target.closest('.carf-add-subfield')) {
                    e.preventDefault();
                    const button = e.target.closest('.carf-add-subfield');
                    const parentIndex = button.dataset.parentIndex;
                    const container = button.parentElement.querySelector('.carf-subfields-container');

                    if (container) {
                        const subfieldsCount = container.querySelectorAll('.carf-subfield-row').length;
                        ...
                        console.error('Subfields container not found');
                    }
                }
            });
            ...
            // Add nested subfield
            document.addEventListener('click', function(e) {
                if (e.target.closest('.carf-add-nested-subfield')) {
                    e.preventDefault();
                    const button = e.target.closest('.carf-add-nested-subfield');
                    const parentIndex = button.dataset.parentIndex;
                    const subIndex = button.dataset.subIndex;
                    const container = button.parentElement.querySelector('.carf-subfields-container');

                    const nestedSubfieldsCount = container.querySelectorAll('.carf-nested-subfield-row').length;
            ...
            // Use a slight delay and check validity to prevent overwriting saved data if PHP rendered it
            // We just trigger the logic to ensure visibility toggles are correct
            setTimeout(function() {
                document.querySelectorAll('.carf-field-type, .carf-subfield-type, .carf-nested-subfield-type').forEach(function(select) {
                ...
                div.innerHTML = `
                    ...
                    <div class="inner_row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 15px;">
                        <div class="inner_colm">
                            <label>${carf_field_group_config.i18n.field_label} *</label>
                            <select name="fields[${index}][type]" class="carf-field-type widefat" required>
                                <option value="text">${carf_field_group_config.i18n.text}</option>
                                <option value="textarea">${carf_field_group_config.i18n.textarea}</option>
                                <option value="image">${carf_field_group_config.i18n.image}</option>
                                <option value="select">${carf_field_group_config.i18n.select}</option>
                                <option value="checkbox">${carf_field_group_config.i18n.checkbox}</option>
                                <option value="radio">${carf_field_group_config.i18n.radio}</option>
                                <option value="color">${carf_field_group_config.i18n.color}</option>
                                <option value="date">${carf_field_group_config.i18n.date}</option>
                                <option value="repeater">${carf_field_group_config.i18n.repeater}</option>
                            </select>
                        </div>
                        
                        <div class="inner_colm">
                            <label>${carf_field_group_config.i18n.field_label} *</label>
                            <input type="text" name="fields[${index}][label]" class="carf-field-label widefat" required placeholder="${carf_field_group_config.i18n.field_label_placeholder}">
                        </div>
                        
                        <div class="inner_colm">
                            <label>${carf_field_group_config.i18n.field_name} *</label>
                            <input type="text" name="fields[${index}][name]" class="carf-field-name widefat" required placeholder="${carf_field_group_config.i18n.field_name_placeholder}">
                            <p class="description">${carf_field_group_config.i18n.field_name_desc}</p>
                        </div>
                    </div>
                    
                    <div class="carf-field-options" style="display: none; margin-bottom: 15px;">
                    </div>
                `;

                const labelInput = div.querySelector('.carf-field-label');
                const nameInput = div.querySelector('.carf-field-name');
            ...
            function createSubfieldRow(parentIndex, subIndex) {
                const div = document.createElement('div');
                div.className = 'carf-subfield-row';
                ...
                div.innerHTML = `
                    <div class="panel_row" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px;">
                        <strong class="top_left_panel">${carf_field_group_config.i18n.sub_field_title}</strong>
                        <div class="top_right_panel">
                             <div class="required_checkbox">
                                <label>
                    ...
                    <div class="inner_row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-bottom: 10px;">
                        <div class="inner_colm">
                            <label>${carf_field_group_config.i18n.field_label} *</label>
                            <select name="fields[${parentIndex}][subfields][${subIndex}][type]" class="widefat carf-subfield-type" required>
                                <option value="text">${carf_field_group_config.i18n.text}</option>
                                <option value="textarea">${carf_field_group_config.i18n.textarea}</option>
                                <option value="image">${carf_field_group_config.i18n.image}</option>
                                <option value="select">${carf_field_group_config.i18n.select}</option>
                        ...
                        <div class="inner_colm">
                            <label>${carf_field_group_config.i18n.field_label} *</label>
                            <input type="text" name="fields[${parentIndex}][subfields][${subIndex}][label]" class="widefat" required placeholder="${carf_field_group_config.i18n.sub_label_placeholder}">
                        </div>
                        
                        <div class="inner_colm">
                            <label>${carf_field_group_config.i18n.field_name} *</label>
                            <input type="text" name="fields[${parentIndex}][subfields][${subIndex}][name]" class="widefat" required placeholder="${carf_field_group_config.i18n.sub_name_placeholder}">
                            <p class="description">${carf_field_group_config.i18n.field_name_desc}</p>
                        </div>
                    </div>
                    
                    <div class="carf-subfield-options" style="display: none; margin-bottom: 10px;">
                    </div>
                `;
            ...
            function createNestedSubfieldRow(parentIndex, subIndex, nestedIndex) {
                const div = document.createElement('div');
                div.className = 'carf-nested-subfield-row';
                div.style.cssText = 'border: 1px solid #ccc; padding: 10px; margin-bottom: 10px; margin-top: 6px; background: #f0f0f0;';
                div.innerHTML = `
                    <div class="panel_row" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px;">
                        <strong  class="top_left_panel">${carf_field_group_config.i18n.nested_sub_field_title}</strong>
                        <div class="top_right_panel">
                            <div class="required_checkbox">
                                <label>
                    ...
                    <div class="inner_row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-bottom: 10px;">
                        <div class="inner_colm">
                            <label>${carf_field_group_config.i18n.field_label} *</label>
                            <select name="fields[${parentIndex}][subfields][${subIndex}][subfields][${nestedIndex}][type]" class="widefat carf-nested-subfield-type" required>
                                <option value="text">${carf_field_group_config.i18n.text}</option>
                                <option value="textarea">${carf_field_group_config.i18n.textarea}</option>
                                <option value="image">${carf_field_group_config.i18n.image}</option>
                                <option value="select">${carf_field_group_config.i18n.select}</option>
                        ...
                        <div class="inner_colm">
                            <label>${carf_field_group_config.i18n.field_label} *</label>
                            <input type="text" name="fields[${parentIndex}][subfields][${subIndex}][subfields][${nestedIndex}][label]" class="widefat" required placeholder="${carf_field_group_config.i18n.nested_label_placeholder}">
                        </div>
                        
                        <div class="inner_colm">
                            <label>${carf_field_group_config.i18n.field_name} *</label>
                            <input type="text" name="fields[${parentIndex}][subfields][${subIndex}][subfields][${nestedIndex}][name]" class="widefat" required placeholder="${carf_field_group_config.i18n.nested_name_placeholder}">
                            <p class="description">${carf_field_group_config.i18n.field_name_desc}</p>
                        </div>
                    </div>
                    
                    <div class="carf-nested-subfield-options" style="display: none; margin-bottom: 10px;">
                    </div>
                `;
            ...
            function updateFieldIndices() {
                const rows = document.querySelectorAll('.carf-field-row');
                rows.forEach(function(row, index) {
                    ...
                    const subfieldsContainer = row.querySelector('.carf-subfields-container');
                    if (subfieldsContainer) {
                        subfieldsContainer.dataset.parentIndex = index;
                    }

                    const addSubfieldBtn = row.querySelector('.carf-add-subfield');
                    if (addSubfieldBtn) {
    ...
    (function($) {
        ...
        function initColorpicker() {
            if (typeof $.fn.wpColorPicker === 'function') {
                $('.carf-colorpicker').each(function() {
        ...
        function initImageUpload() {
            ...
            $(document).on('click', '.carf-upload-button', function(e) {
                e.preventDefault();
                e.stopPropagation();

                var $button = $(this);
                var $container = $button.closest('.carf-file-upload-container');
                var $input = $container.find('.carf-file-input');
                var $preview = $container.find('.carf-file-preview');
                ...
                frame.on('select', function() {
                    ...
                    $.each(attachments, function(i, attachment) {
                        if (attachment.type === 'image') {
                            var fileHtml = '<div class="carf-file-item" data-attachment-id="' + attachment.id + '">' +
                                '<img src="' + attachment.url + '" class="carf-image-preview">' +
                                '<div class="carf-file-name">' + attachment.filename + '</div>' +
                                '<button type="button" class="carf-remove-file dashicons dashicons-no-alt" title="Remove"></button>' +
                                '</div>';
            ...
            // Remove file event
            $(document).on('click', '.carf-remove-file', function(e) {
                e.preventDefault();
                e.stopPropagation();

                var $removeBtn = $(this);
                var $fileItem = $removeBtn.closest('.carf-file-item');
                var $container = $fileItem.closest('.carf-file-upload-container');
                var $input = $container.find('.carf-file-input');
        ...
        function initNestedRepeaters() {
            ...
            function getRowCount($tbody) {
                return $tbody.find('tr[data-nested-index]').not('.carf-clone-nested-row').length;
            }

            // Helper function to update row indices
            // Helper function to update row indices
            function updateRowIndices($tbody, skipFirstRow = false) {
                var rows = $tbody.find('tr[data-nested-index]').not('.carf-clone-nested-row');
                ...
                rows.each(function(newIndex) {
                    ...
                    // Update display index (The Dashicon Number)
                    var $displaySpan = $row.find('.nested-row-index');

                    // If the span doesn't exist yet, create it and append to the handle
                    if ($displaySpan.length === 0) {
                        $displaySpan = $('<span>').addClass('nested-row-index');
                        // Ensure we append to the correct handle element
                        var $handle = $row.find('.carf-row-handle');
                        if ($handle.length > 0) {
                            $handle.append($displaySpan);
                        } else {
                            // Fallback if handle class is missing
                            $row.find('td:first').prepend($displaySpan);
                        }
                    }
                    ...
                    $row.find('[name]').each(function() {
                        ...
                        if (name) {
                            ...
                            // Adjust strictness of split to allow for different nesting depths if necessary
                            var parts = name.split('[');
                            if (parts.length >= 5) {
                                // Check if parts[3] is a number (not a placeholder)
                                var currentPart = parts[3].replace(']', '');
                                // We simply update it if it looks like an index spot
                                parts[3] = newIndex + ']';
            ...
            function fixExistingRows() {
                $('.carf-nested-tbody').each(function() {
                    var $tbody = $(this);
            ...
            $(document).off('click.carAdd').on('click.carAdd', '.carf-add-nested-row', function(e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                var $button = $(this);
                var $nestedRepeater = $button.closest('.carf-nested-repeater');
                var $table = $nestedRepeater.find('.carf-nested-table');
                var $tbody = $table.find('tbody.carf-nested-tbody');
                var $cloneRow = $tbody.find('.carf-clone-nested-row');
                ...
                $newRow.removeClass('carf-clone-nested-row').show();
                ...
                if ($displaySpan.length === 0) {
                    $displaySpan = $('<span>').addClass('nested-row-index');
                    $newRow.find('.carf-row-handle').append($displaySpan);
                }
            ...
            $(document).off('click.carRemove').on('click.carRemove', '.carf-remove-nested-row', function(e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                if (window.carf_ConfirmRemove === undefined) {
                    window.carf_ConfirmRemove = true;
                    ...
                    setTimeout(function() {
                        window.carf_ConfirmRemove = undefined;
                    }, 100);
                }
            ...
            // Sortable for level 1 - STRICT MODE
            if (typeof $.fn.sortable === 'function') {
                $('.carf-nested-tbody').each(function() {
                    var $tbody = $(this);

                    // CRITICAL CHECK: Only apply Level 1 logic if this table contains Level 1 rows
                    // We check this by looking for the Level 1 clone row
                    if ($tbody.find('.carf-clone-nested-row').length > 0) {

                        // Destroy existing sortable if strictly necessary to prevent conflicts
                        ...
                        $tbody.sortable({
                            handle: '.carf-row-handle',
                            axis: 'y',
                            placeholder: 'carf-sortable-placeholder',
                            forcePlaceholderSize: true,
                            items: 'tr[data-nested-index]', // Strictly target Level 1 rows
                            start: function(e, ui) {
                                ui.placeholder.height(ui.item.height());
                            },
                            update: function(event, ui) {
                                console.log('[Custom Advance Repeater] Level 1 Sort detected. Updating indices...');
                                updateRowIndices($(this));
                            }
                        });
                    }
                });
        ...
        function initFieldControls($row) {
            ...
            $row.find('.carf-datepicker').each(function() {
                        ...
                        beforeShow: function(input, inst) {
                            setTimeout(function() {
                                if (inst.dpDiv) {
                                    ...
                                    if ($(input).closest('.carf-nested-table').length) {
                                        inst.dpDiv.css({
                                            'font-size': '12px',
                                            'width': '280px'
                                        });
            ...
            // Color pickers
            $row.find('.carf-colorpicker').each(function() {
        ...
        function initNested2Repeaters() {
            ...
            $(document).on('click', '.carf-add-nested2-row', function(e) {
                e.preventDefault();
                e.stopPropagation();

                var $button = $(this);
                var $nested2Repeater = $button.closest('.carf-nested-repeater');
                var $table = $nested2Repeater.find('.carf-nested-table');
                var $tbody = $table.find('tbody.carf-nested-tbody');
                var $cloneRow = $tbody.find('.carf-clone-nested2-row');
                ...
                var maxIndex = -1;
                $tbody.find('tr:not(.carf-clone-nested2-row)').each(function() {
                ...
                var $newRow = $cloneRow.clone();
                $newRow.removeClass('carf-clone-nested2-row').show();
                ...
                setTimeout(function() {
                    ...
                    $newRow.find('.carf-datepicker').each(function() {
                        if (typeof $.fn.datepicker === 'function' && !$(this).hasClass('hasDatepicker')) {
                            $(this).datepicker({
                                dateFormat: 'yy-mm-dd',
                                changeMonth: true,
                    ...
                    // Color pickers
                    $newRow.find('.carf-colorpicker').each(function() {
                        if (typeof $.fn.wpColorPicker === 'function' && !$(this).hasClass('wp-color-picker')) {
                            $(this).wpColorPicker().addClass('wp-color-picker');
                        }
                    });
            ...
            $(document).on('click', '.carf-remove-nested2-row', function(e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                if (confirm('Are you sure you want to remove this nested row?')) {
                    ...
                    $tbody.find('tr:not(.carf-clone-nested2-row)').each(function(newIndex) {
                        var $currentRow = $(this);
        ...
        function initCAR() {
            if ($('body').hasClass('carf-initialized')) {
            ...
            waitForJQueryUI(function(success) {
                if (success) {
                    ...
                    $('body').addClass('carf-initialized');
